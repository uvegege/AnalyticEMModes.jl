...
@inline is_oblate_basis(::SpheroidalB{I, T, T}) where {I, T} = false
@inline is_oblate_basis(::SpheroidalB{I, T, Complex{T}}) where {I, T} = true

function eval_M(family::Symbol, ξ, η, ϕ, S, dS, R, dR, m, d, even, sph_type)
    if family == :x
        return Mˣ(ξ, η, ϕ, S, dS, R, dR, m, d, even, sph_type)
    elseif family == :y
        return Mʸ(ξ, η, ϕ, S, dS, R, dR, m, d, even, sph_type)
    elseif family == :+ || family == :plus
        return M⁺(ξ, η, ϕ, S, dS, R, dR, m, d, even, sph_type)
    elseif family == :- || family == :minus
        return M⁻(ξ, η, ϕ, S, dS, R, dR, m, d, even, sph_type)
    elseif family == :z
...
function eval_N(family::Symbol, ξ, η, ϕ, S, dS, d²S, R, dR, d²R, m, d, k, even, oblate)
    if family == :x
        return Nˣ(ξ, η, ϕ, S, dS, d²S, R, dR, d²R, m, d, k, even, oblate)
    elseif family == :y
        return Nʸ(ξ, η, ϕ, S, dS, d²S, R, dR, d²R, m, d, k, even, oblate)
    elseif family == :+ || family == :plus
        return N⁺(ξ, η, ϕ, S, dS, d²S, R, dR, d²R, m, d, k, even, oblate)
    elseif family == :- || family == :minus
        return N⁻(ξ, η, ϕ, S, dS, d²S, R, dR, d²R, m, d, k, even, oblate)
    elseif family == :z
...
function evaluate_radial(basis, ξ, radial::Int)
    if radial == 1
...
function compute_radial(basis::SpheroidalBasis{I, T}, ξs, radial::Int) where {I, T}
    if radial == 1
...
"""
    mn_spheroidal_vector(ξ, η, ϕ, mode::SpheroidalB, k;
                         family=:z, even=true, oblate=is_oblate_basis(mode), radial=4)
...
function mn_spheroidal_vector(ξ, η, ϕ, mode::SpheroidalB, k;
                              ...
                              oblate::Bool = is_oblate_basis(mode),
                              radial::Int = 4)
    S, dS, d²S = evaluate_angular(mode, η)
    R, dR, d²R = evaluate_radial(mode, ξ, radial)
    d = mode.c / k
    sph_type = oblate ? :oblate : :prolate
    Mξ, Mη, Mϕ = eval_M(family, ξ, η, ϕ, S, dS, R, dR, mode.m, d, even, sph_type)
    Nξ, Nη, Nϕ = eval_N(family, ξ, η, ϕ, S, dS, d²S, R, dR, d²R, mode.m, d, k, even, oblate)
...
function mn_spheroidal_vectors!(A, points, basis::SpheroidalBasis{I, T}, k;
    ...
    Rm, dRm, d²Rm = compute_radial(basis, ξs, radial)
    ...
    for midx in eachindex(basis.basis)
        ...
        for pidx in eachindex(points)
            ξ, η, ϕ = points[pidx]
            S, dS = Sm[pidx, midx], dSm[pidx, midx]
            R   = Rm[pidx, midx]
            dR  = dRm[pidx, midx]
            d²R = d²Rm[pidx, midx]
            S_r, dS_r, d²S_r = Sm[pidx, midx], dSm[pidx, midx], d²Sm[pidx, midx]
            Mξ, Mη, Mϕ = eval_M(family, ξ, η, ϕ, S, dS, R, dR, b.m, d, even, sph_type)
            Nξ, Nη, Nϕ = eval_N(family, ξ, η, ϕ, S_r, dS_r, d²S_r, R, dR, d²R, b.m, d, k, even, sph_oblate)
