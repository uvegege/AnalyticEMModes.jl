...
function spheroidal_region_mesh(R1, R2; oblate = false, dl = 1e-1, name = "./sphregion.msh")

    Gmsh.gmsh.initialize()
    Gmsh.gmsh.option.setNumber("General.Terminal", 1)
    Gmsh.gmsh.clear()
    Gmsh.gmsh.model.add("ew")
    # Prolate: semi-axes (R2, R2, R1). Oblate: semi-axes (R1, R1, R2).
    if oblate
        sph = Gmsh.gmsh.model.occ.add_sphere(0.0, 0.0, 0.0, R1)
        Gmsh.gmsh.model.occ.dilate([(3, sph)], 0.0, 0.0, 0.0, 1.0, 1.0, R2 / R1)
    else
        sph = Gmsh.gmsh.model.occ.add_sphere(0.0, 0.0, 0.0, R2)
        Gmsh.gmsh.model.occ.dilate([(3, sph)], 0.0, 0.0, 0.0, 1.0, 1.0, R1 / R2)
    end
    Gmsh.gmsh.model.occ.synchronize()
